...
lazy_static! {
    ...
    pub static ref FRONTEND_VERSION: String = {
...
impl<'a> TeraContextBuilder<'a> {
    pub fn build(&mut self) -> TeraContext {
        ...
        if self.show_versions {
            tera_context.insert("backend_version", &get_backend_version().unwrap_or_else(|_| String::from("error")));
            tera_context.insert("frontend_version", &FRONTEND_VERSION.to_string());
        }
...
pub async fn background_worker() {
    loop {
        ...
        if let Err(e) = fetch_backend_version().await {
            write_log(
